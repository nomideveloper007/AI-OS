    ...
    return (
      <div className="p-8 text-center bg-white rounded-2xl border border-slate-200 shadow-2xs space-y-4 max-w-lg mx-auto mt-12 text-xs">
        ...
        <p className="text-xs text-slate-500 font-medium leading-relaxed">
          Mission Control requires an active connected website to orchestrate the AI Agent Workforce and run automated company processes.
        </p>
        <button
  ...
  const startAiCompany = async () => {
    ...
    try {
      ...
      if (mission.status === "failed") {
        ...
        showToast(
          "Mission paused after a stage failure — state preserved. Resume when ready.",
        );
    ...
    <div className="space-y-6 animate-fade-in text-xs">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              One button runs Scanner → Intelligence → CEO → Tasks →
              Collaboration → Runtime → Memory → Reports.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-5 p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs space-y-4">
          ...
          <div>
            <label className="font-extrabold text-slate-600 text-[10px] uppercase">
              Business Goal
            </label>
            <textarea
              value={goal}
          ...
          {selected && (
            <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                onClick={() =>
                  wrap(() => orchestrator.resumeMission(selected.id), "Resumed")
                }
              ...
              <button
                ...
                onClick={() =>
                  wrap(
                    () => orchestrator.restartMission(selected.id),
                    "Restarted",
                  )
                }
