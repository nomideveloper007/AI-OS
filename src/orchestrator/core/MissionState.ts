...
import { MISSION_STAGE_LABELS, MISSION_STAGE_ORDER } from '../types/MissionStage';
import type { MissionProgress, MissionStageRecord } from '../types/MissionProgress';
...
export class MissionState {
  ...
  public static computeProgress(mission: Mission): MissionProgress {
    ...
    const overallPercent =
      mission.status === 'completed'
        ? 100
        : Math.min(99, Math.round((completedStages / Math.max(1, actionable.length)) * 100));

    const started = mission.startedAt ? new Date(mission.startedAt).getTime() : Date.now();
    const elapsedMs = Math.max(0, Date.now() - started);
    const remainingStages = Math.max(0, actionable.length - completedStages);
    const avgPerStage = completedStages > 0 ? elapsedMs / completedStages : 45_000;
  ...
  public static canCancel(mission: Mission): boolean {
    return mission.status === 'running' || mission.status === 'paused' || mission.status === 'pending';
  }
