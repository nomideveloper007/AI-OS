...
export const Sidebar: React.FC = () => {
  const { activeTab, setActiveTab, isSidebarCollapsed, approvals, tasks } =
    useApp();
  ...
  return (
    <aside
      className={`
        ...
        bg-white border-r border-slate-200/80 text-slate-700 select-none overflow-hidden
        ${isSidebarCollapsed ? "w-20" : "w-64"}
      `}
    >
      {/* Top Header & Brand */}
      <div className="p-4 flex items-center gap-3 border-b border-slate-100 flex-shrink-0">
        <div className="w-10 h-10 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[#4F46E5] shadow-2xs flex-shrink-0">
      ...
      <div className="p-3 space-y-2 border-t border-slate-100 flex-shrink-0 bg-white">
        {/* Promotional / AI Agent Assistant Card */}
        {!isSidebarCollapsed && (
          <div className="p-3.5 rounded-2xl bg-gradient-to-b from-slate-50 to-indigo-50/40 border border-slate-100 text-center space-y-2 relative overflow-hidden shadow-2xs">
            <div className="mx-auto w-12 h-12 rounded-xl bg-white p-1 shadow-xs border border-slate-100 flex items-center justify-center relative">
              <svg
                className="w-9 h-9 text-[#4F46E5]"
                viewBox="0 0 64 64"
                fill="none"
              ...
              >
                ...
                />
                <path
                  d="M32 16V8M32 8A3 3 0 1032 2a3 3 0 000 6z"
                  stroke="#4F46E5"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                />
                ...
                <path
                  d="M26 37C28 39.5 36 39.5 38 37"
                  stroke="#4F46E5"
                  strokeWidth="2.5"
                  strokeLinecap="round"
                />
            ...
            <div>
              <h3 className="font-bold text-xs text-slate-900 leading-snug">
                Powerful AI Agents Working 24/7
              </h3>
              <p className="text-[10px] text-slate-500 mt-0.5 leading-tight">
                Automate, Optimize & Grow your website.
              </p>
            </div>
            ...
            <button
              onClick={() =>
                alert(
                  "Upgrade to AI OS Pro to unlock unlimited autonomous agent workers!",
                )
              }
              className="w-full py-2 px-3 rounded-xl bg-[#4F46E5] hover:bg-[#4338CA] text-white font-bold text-xs transition-all shadow-sm flex items-center justify-center cursor-pointer"
            >
              Upgrade Now
            </button>
          </div>
        ...
        {/* User Profile Block */}
        <div className="flex items-center gap-2.5 p-2 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer border border-transparent hover:border-slate-100">
          <img
